...
def define_G(input_nc, output_nc, ngf, which_model_netG,vit_name,img_size,pre_trained_path, norm='batch', use_dropout=False, init_type='normal', gpu_ids=0,pre_trained_trans=True,pre_trained_resnet=0):
    netG = None
    gpu_ids = [gpu_ids]
    ...
    if which_model_netG == 'resnet_9blocks':
        ...
        netG = residual_transformers.Res_CNN(residual_transformers.CONFIGS[vit_name], input_dim= input_nc, img_size=img_size, output_dim=output_nc, vis=False)
    elif which_model_netG == 'resvit':
        print(vit_name)
        netG = residual_transformers.ResViT(residual_transformers.CONFIGS[vit_name],input_dim = input_nc,img_size=img_size, output_dim=output_nc, vis=False)
...
def define_D(input_nc, ndf, which_model_netD,vit_name,img_size,
             n_layers_D=3, norm='batch', use_sigmoid=False, init_type='normal', gpu_ids=0):
    netD = None
    gpu_ids = [gpu_ids]
    use_gpu = len(gpu_ids) > 0
    norm_layer = get_norm_layer(norm_type=norm)
...
class GANLoss(nn.Module):
    def __init__(self, use_lsgan=True, target_real_label=1.0, target_fake_label=0.0,
                 tensor=torch.FloatTensor, device='cuda:0'):
...
class Encoder_Decoder(nn.Module):
    ...
    def forward(self, input):
        #if self.gpu_ids and isinstance(input.data_example, torch.cuda.FloatTensor):
        encoded=[]
